#!/usr/bin/env python3
"""Analyze our model's computation to understand the difference."""
...
# Run our model at step 0
print("\n--- Step 0 ---")
output_0 = our_decoder.predict({
    "encoder_hidden_states": encoder_hidden.astype(np.float32),
    "input_id": np.array([[13764]], dtype=np.int32),
    "cache_k": np.zeros((8, 8, 108, 128), dtype=np.float32),
    "cache_v": np.zeros((8, 8, 108, 128), dtype=np.float32),
...
# Run our model at step 3
print("\n--- Step 3 ---")
output_3 = our_decoder.predict({
    "encoder_hidden_states": encoder_hidden.astype(np.float32),
    "input_id": input_id,
    "cache_k": cache_k.astype(np.float32),
    "cache_v": cache_v.astype(np.float32),
    "step": step,
    "cross_attention_mask": cross_mask.astype(np.float32),
})
